...
interface MediationRequestForm {
  ...
  mediatorFeeType: 'paid' | 'free' // 调解员收费类型：收费或免费
...
const formData = ref<MediationRequestForm>({
  ...
  respondentPdfFiles: [],
  mediatorSelection: 'system',
  selectedMediator: '刘宇',
  mediatorFeeType: 'paid', // 默认选择收费调解员
...
// 处理调解员选择变更
function handleMediatorSelectionChange(e: any) {
  const value = e.detail.value
  console.log('调解员选择模式变更为:', value)

  // 更新formData中的选择值
  formData.value.mediatorSelection = value

  // 当选择在线选择时，跳转到调解机构列表页面
  if (value === 'online') {
    setTimeout(() => {
      uni.navigateTo({
        url: '/pages-sub/1-dispute-mediation/mediation-agency-list',
      })
    }, 100)
  }
}
...
onLoad((options: any) => {
  ...
  if ('mediatorName' in options) {
    ...
    try {
      ...
      formData.value.mediatorSelection = 'online' // 有指定调解员时设为在线选择
    ...
    console.log('未检测到mediatorName参数，使用系统指定')
    formData.value.mediatorSelection = 'system' // 没有指定调解员时设为系统指定
  }
    ...
    <view class="form-content">
      ...
      <view class="form-section">
        ...
        <view class="form-item">
          ...
          <picker class="form-picker" :range="['合同纠纷', '房产纠纷', '婚姻家庭纠纷', '劳动纠纷', '交通事故纠纷', '知识产权纠纷', '其他纠纷']" @change="(e) => formData.caseType = ['合同纠纷', '房产纠纷', '婚姻家庭纠纷', '劳动纠纷', '交通事故纠纷', '知识产权纠纷', '其他纠纷'][e.detail.value]">
      ...
      <view class="form-section">
        ...
        <view class="form-item">
          ...
          <picker class="form-picker" :range="['合同纠纷', '房产纠纷', '婚姻家庭纠纷', '劳动纠纷', '交通事故纠纷', '知识产权纠纷', '其他纠纷']" @change="(e) => formData.respondentCaseType = ['合同纠纷', '房产纠纷', '婚姻家庭纠纷', '劳动纠纷', '交通事故纠纷', '知识产权纠纷', '其他纠纷'][e.detail.value]">
            <view class="picker-placeholder">
      ...
      <view class="form-section">
        ...
        <view class="form-item">
          ...
          <view class="mediator-selection">
            <radio-group v-model="formData.mediatorSelection" @change="handleMediatorSelectionChange">
          ...
          <!-- 系统指定时显示收费类型选择 -->
          <view v-if="formData.mediatorSelection === 'system'" class="mediator-fee-selection">
            <view class="fee-selection-label">
              请选择您需要免费调解员还是收费调解员
            </view>
            <radio-group v-model="formData.mediatorFeeType">
              <view class="radio-item">
                <radio value="paid" />收费调解员
              </view>
              <view class="radio-item">
                <radio value="free" />免费调解员
              </view>
            </radio-group>
          </view>

          <!-- 在线选择时显示已选调解员信息 -->
          <view v-else-if="formData.mediatorSelection === 'online'" class="selected-mediator">
